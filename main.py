...
def send_trade_signal(force=False):
    ...
    # Пропустить если сигнал не принудительный и прошло <1ч и вероятность <90%
    if not force and probability < 90 and now - last_signal_time < 3600:
...
def auto_signal_loop():
    while True:
        try:
            send_trade_signal(force=False)
        except Exception as e:
            print(f"❌ Ошибка в авто-сигнале: {e}")
...
if __name__ == "__main__":
    ...
    # Запускаем авторассылку в отдельном потоке
